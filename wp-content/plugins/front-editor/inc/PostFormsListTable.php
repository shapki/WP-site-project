 ...
 * @since 2.5
 */
class PostFormsListTable extends WP_List_Table {
    ...
    public function get_views() {
        ...
        $post_statuses  = apply_filters( 'fe_post_forms_list_table_post_statuses', [
            'all'       => __( 'All', FE_TEXT_DOMAIN),
            'publish'   => __( 'Published', FE_TEXT_DOMAIN),
            'trash'     => __( 'Trash', FE_TEXT_DOMAIN),
        ] );
        ...
        foreach ( $post_statuses as $status => $status_title ) {
            $link = ( 'all' === $status ) ? $base_link : admin_url( 'admin.php?page=fe-post-forms&post_status=' . $status );
        ...
        return apply_filters( 'fe_post_forms_list_table_get_views', $status_links, $post_statuses, $current_status );
    }
    ...
    public function no_items() {
        esc_html_e( 'No form found.', FE_TEXT_DOMAIN);
    }
    ...
    public function get_bulk_actions() {
        ...
        //     $actions['trash'] = __( 'Move to Trash', FE_TEXT_DOMAIN);
        // }

        // if ( isset( $_GET['post_status'] ) && 'trash' === $_GET['post_status'] ) {
        //     $actions['restore'] = __( 'Restore', FE_TEXT_DOMAIN);
        //     $actions['delete']  = __( 'Delete Permanently', FE_TEXT_DOMAIN);
        // }
    ...
    public function search_box( $text, $input_id ) {
        ...
        do_action( 'fe_post_forms_list_table_search_box', $text, $input_id );
    ...
    public function item_query( $args ) {
        ...
        if ( $query->have_posts() ) {
            ...
            while ( $query->have_posts() ) {
                ...
                $settings = get_post_meta( get_the_ID(), 'fe_form_settings', true );

                $forms[ $i ] = [
                    'ID'                    => $form->ID,
                    'post_title'            => $form->post_title,
                    'post_status'           => $form->post_status,
                    'settings_post_type'    => isset( $settings['fe_post_type'] ) ? $settings['fe_post_type'] : '',
                    'settings_post_status'  => isset( $settings['fe_post_status'] ) ? $settings['fe_post_status'] : '',
        ...
        $forms = apply_filters( 'fe_post_forms_list_table_query_results', $forms, $query, $args );
    ...
    public function get_columns() {
        $columns = [
            ...
            'form_name'     => __( 'Form Name', FE_TEXT_DOMAIN),
            'post_type'     => __( 'Post Type', FE_TEXT_DOMAIN),
            'post_status'   => __( 'Post Status', FE_TEXT_DOMAIN),
            'guest_post'    => __( 'Guest Post', FE_TEXT_DOMAIN),
            'shortcode'     => __( 'Shortcode', FE_TEXT_DOMAIN),
        ];

        return apply_filters( 'fe_post_forms_list_table_cols', $columns );
    }
    ...
    public function get_sortable_columns() {
        ...
        return apply_filters( 'fe_post_forms_list_table_sortable_columns', $sortable_columns );
    }
    ...
    public function column_default( $item, $column_name ) {
        switch ( $column_name ) {
            ...
            case 'post_status':
                return fe_admin_post_status( $item['settings_post_status'] );

            case 'guest_post':
                $is_guest_post  = $item['settings_guest_post'];
                $url            = FE_PLUGIN_URL . '/assets/img/';
                $image          = '<img src="%s" alt="%s" style="%s">';
                $style = 'width: 20px;display: block;';

                return filter_var( $is_guest_post, FILTER_VALIDATE_BOOLEAN ) ?
                            sprintf( $image, $url . 'check.svg', __( 'Yes', FE_TEXT_DOMAIN),$style ) :
                            sprintf( $image, $url . 'cross.svg', __( 'No', FE_TEXT_DOMAIN),$style );
    ...
    public function column_form_name( $item ) {
        ...
        $edit_url       = admin_url( 'admin.php?page=fe-post-forms&action=edit&id=' . $item['ID'] );
        $wpnonce        = wp_create_nonce( 'bulk-post-forms' );
        $admin_url      = admin_url( 'admin.php?page=fe-post-forms&id=' . $item['ID'] . '&_wpnonce=' . $wpnonce );
        ...
        if ( ( !isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) && current_user_can( fe_admin_role() ) ) {
            $actions['edit']        = sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', FE_TEXT_DOMAIN) );
            $actions['trash']       = sprintf( '<a href="%s" class="submitdelete">%s</a>', $trash_url, __( 'Trash', FE_TEXT_DOMAIN) );
            $actions['duplicate']   = sprintf( '<a href="%s">%s</a>', $duplicate_url, __( 'Duplicate', FE_TEXT_DOMAIN) );
        ...
        if ( ( isset( $_GET['post_status'] ) && 'trash' === $_GET['post_status'] ) && current_user_can( fe_admin_role() ) ) {
            $actions['restore'] = sprintf( '<a href="%s">%s</a>', $restore_url, __( 'Restore', FE_TEXT_DOMAIN) );
            $actions['delete']  = sprintf( '<a href="%s" class="submitdelete">%s</a>', $delete_url, __( 'Delete Permanently', FE_TEXT_DOMAIN) );
        ...
        $draft_marker = ( 'draft' === $item['post_status'] ) ?
                            '<strong> — <span class="post-state">' . __( 'Draft', FE_TEXT_DOMAIN) . '</span></strong>' :
                            '';

        $form_name = sprintf( '%1s %2s %3s', $title, $draft_marker, $this->row_actions( $actions ) );

        return apply_filters( 'fe_post_forms_list_table_column_form_name', $form_name, $item );
    }
}
